...
public class MovingObject extends GameObject{
	...
	public void collisionAvoid(){
		for(GameObject o : GameObject.gameObjects){
			if(o != this) sweepOverlaps(o);
		}
	}
	
	protected void sweepOverlaps(GameObject b){
		...
		if(tom > 1) return;									// if > 1 it won't happen in this frame
       ...
       //calculate time of y overlap end           
		...
		if(tom >= tsm) return;								// if we don't overlap before we separate, then we didn't collide
		
		// calculate the avoidance velocity
		if(to[0] > to[1]) velocity.x = (long) (velocity.x * tom + b.velocity.x * (tsm - tom));
		else velocity.y = (long)(velocity.y * tom + b.velocity.y * (tsm - tom));
	}
}